...
Regarding the QR code, the specific technology are going to use, it have been invented in 1994 for industrial applications. The current version, described in \cite{isoqr}, is dated February 2015. In 2013 an early paper by Martinez et al. \cite{mar13a} suggests its utilization to implement a virtual tour in the {\em Las Quilamas Natural Park} in Spain. At that time the availability of QR-code readers was not as diffused as today, and therefore the work may be considered among the seminal ones. The authors proposed to use the QR code as a link to a panel in PDF format viewed on the user's portable device. A similar work in an urban environment is presented in \cite{fin13a} and \cite{tat15a}, the latter illustrating the QR system serving the monuments in the Serbian town of Niš.
...
All the referenced publications share a common attitude, which can be summarized in the motto "analog portal to the digital world", as in the title of \cite{bai12a}. None of them evaluates the possibility of storing text in the tag itself, thus eliminating the requirement of an Internet connection.

The present paper promotes the use of QR codes as the primary or only way of providing information to the visitor. Unlike in the literature, where QR tags are considered ancillary to other forms of communication, like physical installations or linked Web contents. Therefore, in our case the informational content of the tag, which is commonly ignored, becomes relevant. It is a new and original way of using a QR code for the delivery of information.
...
Previous investigations, including archaeological surveys and access to historical documents and cartography, allowed to design of the contents to be delivered through signage. Two on-site surveys provided further practical indications and the opportunity to place the QR tags.
